...
function get_inputs(n; latent_dist = Uniform(-1, 1), ld=2)
    ctes = Float32.(rand(Uniform(-10 / 6.366468343804353, 10 / 6.366468343804353), 1, n))
    hes = Float32.(rand(Uniform(-30 / 17.248858791583547, 30 / 17.248858791583547), 1, n))
    ls = Float32.(rand(latent_dist, ld, n))
