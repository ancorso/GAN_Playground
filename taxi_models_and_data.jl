...
function TaxiDConv(s::Settings)
	...
	d_common = Chain(
					 ...
					 BatchNorm(64, leakyrelu),
			 		 Conv((4,4), 64=>64, pad=1, stride=2, leakyrelu),
					 BatchNorm(64, leakyrelu),
					 Conv((3,3), 64=>128, pad=1, stride=1, leakyrelu),
					 BatchNorm(128, leakyrelu),
					 Conv((4,4), 128=>128, pad=1, stride=2, leakyrelu),
					 BatchNorm(128, leakyrelu),
					 Conv((3,3), 128=>256, pad=1, stride=1, leakyrelu),
					 BatchNorm(256, leakyrelu),
					 Conv((4,4), 256=>256, pad=1, stride=2, leakyrelu),
					 BatchNorm(256, leakyrelu),
			  		 Conv((3,3), 256=>512, pad=1, stride=1, leakyrelu),
			  		 x -> reshape(x, 1024, :),
			  		 Dense(1024, 1)
					 ) |> gpu
...
images = h5read("SK_DownsampledGANFocusAreaData.h5", "X_train")


train(Settings(G = TaxiGConv, D = TaxiDConv, rand_input = Taxi_input, loss = DCGANLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_DCGAN"))
train(Settings(G = TaxiGConv, D = TaxiDConvSpectral, rand_input = Taxi_input, loss = DCGANLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_DCGAN_Spectral"))

train(Settings(G = TaxiGConv, D = TaxiDConv, rand_input = Taxi_input, loss = LSLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_LSGAN"))
train(Settings(G = TaxiGConv, D = TaxiDConvSpectral, rand_input = Taxi_input, loss = LSLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_LSGAN_Spectral"))

train(Settings(G = TaxiGConv, D = TaxiDConv, rand_input = Taxi_input, loss = HingeLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_Hinge"))
train(Settings(G = TaxiGConv, D = TaxiDConvSpectral, rand_input = Taxi_input, loss = HingeLoss(), img_fun = gen_Taxi_images, nclasses = 2, output_dir = "Taxi_Hinge_Spectral"))
